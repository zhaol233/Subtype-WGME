...
class ViTEncoder(nn.Module):
    def __init__(self,
        ...
        # easy MLP
        # self.encoder =Mlp(self.length, latent_dim, latent_dim, drop=0.2)
        self.encoder = nn.Linear(self.length,latent_dim)
...
class ViTDecoder(nn.Module):
    def __init__(self, data_type, data_size, img_size=224, latent_dim=256):
        ...
        # img decoder
        # self.vit_x = nn.Linear(latent_dim, self.length)
        self.vit_x = nn.Sequential(
            Mlp(latent_dim,
                ...
                act_layer=nn.ReLU),
            nn.Linear(latent_dim, latent_dim),
            nn.Linear(latent_dim, self.length),
