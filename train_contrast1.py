...
from model_contrast1 import SubtypeWGME, ValidModel, loss_kd
import os
...
def run_train_WGME(cancer_type):
    ...
    if args.valid:
        ...
        vmodel = vmodel.cuda()
        model_path = f"../results/model_contrast1/{cancer_type}.pt"
    ...
    #     epochs = base + increase_base * increase_exp
    for epoch in range(epochs + 1):
        ...
        if epoch % 5 == 0:
            ...
            log.info(
                f"epoch: {epoch} loss0:{round(float(loss0.cpu()), 4)} loss_real: {loss_real} loss fake: {loss_fake}"
            )
...
def run_biomarker_xgboost(cancer_type):
    data = load_data(cancer_type, tensor=False)
    X = pd.concat(data[0], axis=1)
    feature = list(X.columns)
    labels = pd.read_csv(f'../results/{cancer_type}/{cancer_type}.SubtypeWGME',
                         index_col=0)
    labels['subtype'] = labels['subtype'] - 1
    y = labels.loc[X.index, :]

    omics_data_type = data[2]
    omics_data_size = data[3]
    model = XGBClassifier()
...
def run():
    ...
    # if args.run_mode == 'biomarker_shap':
    #     run_biomarker_shap(cancer_type)
